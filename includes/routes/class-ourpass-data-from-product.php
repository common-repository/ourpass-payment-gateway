...
class OurPass_Routes_OurPass_Data_From_Product extends OurPass_Routes_Route {
	...
	public function callback( $request ) {
        try {
            ...
            $productData['url'] = site_url();
            //SRC
            $src = wp_get_attachment_image_url($product->get_image_id(), 'full');
            $linkParts = explode('?', $src);
            $productData['src'] = isset($linkParts[0]) ? $linkParts[0] : '';
            //METADATA
            ...
            $referenceCode = OURPASSWC_ORDER_PREFIX.''. md5(uniqid(bin2hex(random_bytes(20)), true)) . '' . ourpasswc_get_last_sale_order_post_id();

            $ourpass_data = array();

            $ourpass_data['env'] = ourpasswc_get_environment();
            $ourpass_data['api_key'] = ourpasswc_get_secret_key();
            $ourpass_data['sub_account_key'] = ourpasswc_get_sub_account_key();
