...
class OurPass_Routes_OurPass_Response_Create_Order extends OurPass_Routes_Route {
	...
	public function callback( $request ) {
        try {
            ...
            error_log("OurPass Checkout: ". $error->getMessage());
			return new \WP_REST_Response([
				'success' => false,
    ...
    {
        $baseUrl = ourpasswc_get_environment() === 'live' ? OURPASSWC_PRODUCTION_LIVE_BASE_URL : OURPASSWC_PRODUCTION_SANDBOX_BASE_URL;
        ...
        $response = wp_remote_post($ourpass_url, $args);
    ...
    {
        $len = strlen(OURPASSWC_ORDER_PREFIX);
        return (substr($reference, 0, $len) === OURPASSWC_ORDER_PREFIX);
    }
}
